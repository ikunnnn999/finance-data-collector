...
def simulate(returns, rf, targets, cost_bps=10., rebalance='daily', sell_cost_bps=None):
    ...
    if sell_cost_bps is None:
        sell_cost_bps = pd.DataFrame(0., index=returns.index, columns=returns.columns)
    if not sell_cost_bps.index.equals(returns.index) or not sell_cost_bps.columns.equals(returns.columns):
        raise ValueError('Sell cost dates and assets must match')
    if not np.isfinite(sell_cost_bps).all().all() or (sell_cost_bps < 0).any().any() or (sell_cost_bps >= 1000).any().any():
        raise ValueError('Invalid sell cost rates')
    ...
    for i, (date, r) in enumerate(returns.iterrows()):
        ...
        sell_rates = sell_cost_bps.loc[date].to_numpy() / 10000
        def fee(c):
            delta = (1 - c) * target - drift
            return cost_rate * np.abs(delta).sum() + sell_rates @ np.maximum(-delta, 0)
        cost = brentq(lambda c: c - fee(c), 0, 2 * cost_rate + sell_rates.max() + 1e-12) if cost_rate or sell_rates.max() else 0.
        sell_cost = float(sell_rates @ np.maximum(drift - (1 - cost) * target, 0))
        ...
        records.append(dict(date=date, gross_return=gross - 1, net_return=net, wealth=wealth,
                            cost_fraction=cost, sell_cost_fraction=sell_cost, turnover=turnover, exposure=target.sum(),
