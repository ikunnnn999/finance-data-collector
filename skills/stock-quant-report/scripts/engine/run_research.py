...
def main():
    ...
    parser.add_argument('--symbols', nargs='+', help='US asset universe; benchmark is included automatically')
    parser.add_argument('--primary', help='Asset for factor regression and timing strategies')
    ...
    symbols = list(dict.fromkeys((args.symbols or ['AAPL', 'MSFT', 'SPY']) + [args.benchmark]))
    primary = args.primary or symbols[0]
    if primary not in symbols:
        raise ValueError('--primary must be in --symbols')
    ...
    for name, (path, factors) in snapshots.items():
        ...
        command = [sys.executable, str(ROOT / 'indicators/fama_french.py'), '--stock', str(args.raw_dir / f'{primary}.csv'),
    ...
    target = pd.DataFrame(0., index=test.index, columns=symbols)
    target[primary] = moving_average_targets(prices[primary], test.index, args.fast, args.slow)
    results[primary + '_moving_average'] = simulate(test, rf.loc[test.index], target, args.cost_bps)
    prediction_scores = None
    if args.include_ml:
        predictions, prediction_scores, folds = walk_forward_direction(prices[primary], returns[primary], test.index)
        ...
        target = pd.DataFrame(0., index=test.index, columns=symbols)
        target[primary] = predictions.predicted_up
        results[primary + '_logistic'] = simulate(test, rf.loc[test.index], target, args.cost_bps)
    metrics = []
    ...
    report = [f'# {primary} 量化投资与资产定价研究报告', '',
              ...
              f'预先选择的资产池（{", ".join(symbols)}）存在选择偏差。结果为研究原型诊断，不代表可实现的投资业绩。', '',
